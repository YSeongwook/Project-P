...
public class AirShipMove : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Vector3 direction = airShipPos[currentPosIndex].position - transform.position;
        transform.position += direction.normalized * airShipSpeed * Time.deltaTime;

        // 방향을 부드럽게 회전 (Slerp 사용)
        transform.rotation = Quaternion.Slerp(transform.rotation, airShipDir, rotationSpeed * Time.deltaTime);

        // 목표 위치에 거의 도달했는지 확인
        if (Vector3.Distance(transform.position, airShipPos[currentPosIndex].position) < 10f)
        {
            // 다음 목표 위치로 전환
            currentPosIndex++;
